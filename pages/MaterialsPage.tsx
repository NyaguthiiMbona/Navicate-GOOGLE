      ...
      <div className="max-w-6xl mx-auto px-4 py-16">
        ...
        {limitReached && !materials && showPreview && !loading && (
          <div className="mb-12 p-8 bg-slate-900/50 border border-slate-800 rounded-xl text-center animate-in fade-in duration-500">
            <h3 className="text-white font-bold mb-2">Generation Limit Reached</h3>
            <p className="text-sm text-slate-400 mb-6 max-w-lg mx-auto leading-relaxed">
              You've reached your free daily generation limit. You can still browse other roles or edit your background, but new document generation is restricted until tomorrow.
            </p>
            <Link to="/pricing">
          ...
          <div className="max-w-2xl mx-auto animate-in fade-in zoom-in duration-300">
            <div className="bg-slate-900/50 border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-slate-800 bg-slate-900/30">
                <h2 className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-8 text-center">Generation Preview</h2>
                <div className="space-y-10">
                  <div className="grid grid-cols-2 gap-8">
                    <div>
                      <label className="text-[10px] font-bold text-[#D4AF37] uppercase block mb-2 tracking-widest">Active Role</label>
                      <p className="text-sm text-white font-bold">{targetRole}</p>
                    </div>
                    <div>
                      <label className="text-[10px] font-bold text-[#D4AF37] uppercase block mb-2 tracking-widest">Experience level</label>
                      <p className="text-sm text-white font-bold">{sessionBackground?.yearsOfExperience} Years Experience</p>
                    </div>
                  </div>
                  
                  {pathData?.transferableSkills && (
                    <div>
                      <label className="text-[10px] font-bold text-[#D4AF37] uppercase block mb-3 tracking-widest">Key Detected Skills</label>
                  ...
                  <div className="p-4 bg-black/30 rounded border border-slate-800/50">
                     <label className="text-[10px] font-bold text-slate-500 uppercase block mb-2 tracking-widest">Output logic</label>
                     <p className="text-xs text-slate-400 italic leading-relaxed">
                      {type === 'cv' 
                        ? `Professional CV draft tailored for ${targetRole}. Focus: ${sessionBackground?.yearsOfExperience && sessionBackground.yearsOfExperience <= 2 ? 'Academic achievements and functional projects' : 'Strategic impact and leadership milestones'}.` 
                        : "Unique cover letter mapped strictly to your established background. Conservative and professional phrasing."
                      }
                     </p>
                  </div>
                </div>
              ...
              <div className="p-8 bg-black/20 flex flex-col items-center">
                <Button 
                  ...
                  className={limitReached ? 'cursor-not-allowed opacity-40' : ''}
                >
                  {limitReached ? 'Daily Limit Reached' : `Generate ${type === 'cv' ? 'CV' : 'Cover Letter'}`}
                </Button>
                <p className="mt-4 text-[10px] text-slate-500 uppercase font-bold tracking-widest">
                  {limitReached ? 'Upgrade for additional access' : 'Counts as one daily generation'}
                </p>
              </div>
            </div>
            ...
            <div className="mt-8 text-center">
              <p className="text-[10px] text-slate-600 uppercase tracking-widest leading-relaxed">
                Navicate results are based on market patterns and user input. <br />
        ...
        {loading && (
          <div className="py-32 text-center">
            <div className="max-w-sm mx-auto space-y-6">
              ...
              <p className="text-white text-sm font-bold">Reframing background for {targetRole}...</p>
              <p className="text-[10px] text-slate-500 italic uppercase tracking-widest">Applying {sessionBackground?.experienceLevel} context</p>
            </div>
          </div>
        )}
        ...
        {error && !loading && (
          <div className="bg-red-900/10 border border-red-900/30 p-8 rounded-lg text-center max-w-md mx-auto">
            ...
            <Button onClick={handleTriggerGeneration} className="text-xs">Retry Generation</Button>
          </div>
        )}
        ...
        {materials && !loading && (
          <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="max-w-4xl mx-auto">
               <div className="bg-emerald-500/10 border border-emerald-500/20 px-4 py-2 rounded-md mb-8 inline-block">
                 <p className="text-xs text-emerald-500 font-bold uppercase tracking-widest">
                  {type.toUpperCase()} generated for {targetRole}
                 </p>
               </div>
              ...
              {materials.cv && type === 'cv' && (
                <div className="bg-slate-900/30 border border-slate-800 p-8 rounded-xl space-y-10">
                  <div className="border-b border-slate-800 pb-4 flex justify-between items-center">
                    <h2 className="text-[10px] font-bold text-white uppercase tracking-widest">Tailored CV Draft</h2>
                    <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">ATS Optimized</span>
                  </div>
                  <div className="space-y-8">
                    <div className="group">
                      <label className="text-[10px] font-bold text-[#D4AF37] uppercase block mb-3 tracking-widest opacity-60">Professional Summary</label>
                      <div className="w-full bg-slate-950/50 border border-slate-800 rounded-lg p-5 text-sm text-slate-300 leading-relaxed min-h-[100px] whitespace-pre-wrap">
                        {materials.cv.summary}
                      </div>
                    </div>
                    <div className="group">
                      <label className="text-[10px] font-bold text-[#D4AF37] uppercase block mb-3 tracking-widest opacity-60">Reframed Experience</label>
                    ...
                    <div className="group">
                      <label className="text-[10px] font-bold text-[#D4AF37] uppercase block mb-3 tracking-widest opacity-60">Mapped Skills</label>
                      <div className="flex flex-wrap gap-2">
                        {materials.cv.skills.map((skill, idx) => (
                          <div key={idx} className="bg-slate-950 border border-slate-800 px-3 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-wider text-slate-400 border-l-2 border-l-[#D4AF37]/40">{skill}</div>
                        ))}
                      </div>
                    </div>
                  </div>
              ...
              {materials.coverLetter && type === 'coverLetter' && (
                <div className="bg-slate-900/30 border border-slate-800 p-8 rounded-xl space-y-8">
                  <div className="border-b border-slate-800 pb-4 flex justify-between items-center">
                    <h2 className="text-[10px] font-bold text-white uppercase tracking-widest">Unique Cover Letter</h2>
                    <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Aligned with CV</span>
                  </div>
                  <div className="bg-slate-950/50 border border-slate-800 rounded-lg p-8 text-sm text-slate-300 leading-relaxed min-h-[500px] whitespace-pre-wrap font-serif">
                    {materials.coverLetter}
                  </div>
                  <p className="text-[10px] text-slate-500 italic uppercase tracking-widest">Language is unique per generation for {targetRole}.</p>
                </div>
              )}

              <div className="mt-12 flex flex-col sm:flex-row gap-4 justify-center">
                <Button onClick={() => window.print()} variant="outline" className="text-xs px-10">Save as PDF</Button>
                <Button onClick={handleRegenerate} variant="secondary" className="text-xs px-10" disabled={limitReached && !materials}>
                  Regenerate {type.toUpperCase()}
                </Button>
                <Link to="/explore"><Button variant="primary" className="text-xs px-10">Explore Other Roles</Button></Link>
              </div>
              
              <div className="mt-12 pt-8 border-t border-slate-900 text-center">
                <p className="text-[10px] text-slate-600 uppercase tracking-widest leading-relaxed max-w-md mx-auto">
                  Verify all details before submission. Personal oversight is required for all AI-assisted materials.
                </p>
              </div>
            </div>
          </div>
